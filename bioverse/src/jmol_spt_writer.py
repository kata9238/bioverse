...
class JmolSPTWriter:
    def __init__(self, pdb_code, tag=''):
        ...
        self.spt_filename = ("/Users/andrea/repositories/bioverse_beta/"
                             "bioverse/static/results/jmol_script.spt")
        self.tag = tag
    ...
    def write_spt(self):
        ...
        if self.tag == '':
            filename = \
                self.spt_filename.split('.')[0] + '_' + self.filename + '.spt'
        else:
            filename = (self.spt_filename.split('.')[0] + '_' + self.filename +
                        self.tag + '.spt')
            ...
            if len(fields) == 5:
                if fields[2] == 'load':
                    ...
                    if self.tag == '':
                        ...
                        new_contents[-1] = \
                            (self.filename + self.tag).strip() + '.pdb";'
