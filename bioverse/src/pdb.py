...
class PDBFromUniprot:
	def __init__(self):
		self.pdb_dir = os.getcwd() + '/bioverse/static/results/' ## running from server
	...
	def _isempty_pdb(self, pdb_code):
		...
		#print pdb_files
		#print file_names
		#if pdb_code.upper() in file_names:
		#	print pdb_file
...
class CIFFParser:
	...
	def get_gene_info_from_genbank(self):
		...
		for short_gene_name in self.filtered_pdb_genes:
			for feature in genome.features:
				if feature.type == 'CDS' and short_gene_name in [gene.upper() for gene in feature.qualifiers['gene']]:
					gene_info[short_gene_name] = {}
					try:
						if 'gene' in feature.qualifiers:
							gene_info[short_gene_name]['GENE'] = feature.qualifiers['gene']
						else:
					...
					try: 
						if 'product' in feature.qualifiers:
							gene_info[short_gene_name]['PRODUCT'] = feature.qualifiers['product']
						else:
					...
					try:
						if 'function' in feature.qualifiers:
							gene_info[short_gene_name]['FUNCTION'] = feature.qualifiers['function']
						else:
					...
					try:
						if 'protein_id' in feature.qualifiers:
							gene_info[short_gene_name]['PROTEIN_ID'] = feature.qualifiers['protein_id']
						else:
		...
		for pdb_file in pdb_files:
			...
			if ( self.filename ) == file_handlers.get_file_name(pdb_file).split('.')[0]:
				for gene, chain in zip(genes, chains):
					#print gene, chain
					cmd = ['python ~/repositories/bioverse_beta/bioverse/src/get_fasta_from_pdb.py ' + pdb_file + ' ' + chain + ' ' + (gene + '_' + self.filename + '_chain-' + chain + '.fasta')] 
...
class LigandBindingSite:
	...
	def write_residue_output(self):
		active_site_filename = self.pdb_dir + '/' + self.filename + '_lpocket.txt'
	...
	def _edit_bfactor(self, file_tag):
		...
		for line in raw_pdb:
			if line[0:6] == "ATOM  ":
				...
				if chain in offset and (name, chain, (resnum - offset[chain])) in self.data_dict.keys():
					lines_out.append("%s%6.2F%s" % (line[:60], self.data_dict[(name, chain, (resnum - offset[chain]))], line[66:]))
				else:
