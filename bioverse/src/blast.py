...
class wwwBLAST:
    ...
    def blast_record(self):
        ...
        result_handle = NCBIWWW.qblast(
            "blastp", "nr", self.genbank_record_number)
    ...
    def save_blast_xml(self):
        print ("Saving BLAST result (.xml) for record "
               "number %d" % int(self.genbank_record_number))
...
class localBLAST:
    ...
    def _pairwise_blast(self, query_files, subject_files, protein=False,
                        nucleotide=False):
        file_handlers = FileHandlers()
        blast_command = 'blastp' if protein else 'blastn'
        blast_result_files = []
        for query, subject in zip(query_files, subject_files):
            blast_result = \
                (file_handlers.get_file_name(query).split('.')[0] +
                 file_handlers.get_file_name(subject).split('.')[0] +
                 '_blastp.out')
            cmd = [blast_command + ' -query ' + query + ' -subject ' +
                   subject + ' > ' + blast_result]
    ...
    def _get_fasta_file_paths(self):
        ...
        for i in range(len(self.sequence_annotations)):
            organism_id = \
                settings.ORGANISM_MAP[self.sequence_annotations[i][2].upper()]
            file_handlers = FileHandlers()
            file_paths = file_handlers.search_directory()
            fasta_files = file_handlers.find_files(file_paths, 'fasta')
            for fasta_file in fasta_files:
                if self.sequence_annotations[i][1] + '_' + organism_id == \
                        file_handlers.get_file_name(
                            fasta_file).split('.fasta')[0]:
                    query_files.append(fasta_file)
                elif (self.sequence_annotations[i][1] + '_' + self.filename +
                      '_chain-' + self.sequence_annotations[i][0]) == \
                        file_handlers.get_file_name(fasta_file).split('.')[0]:
            ...
            while pdb_start == '':
                for line in cleaned_data:
                    if len(line) == 4 and line[0] == 'Sbjct':
                        ...
                        pass
            seq_name = (result_file.split('_')[0] + '_' +
                        result_file.split('_')[3] + '_' +
                        result_file.split('_')[4])
