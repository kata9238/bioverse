...
<macromoleculeName>
    ...
    def url_for_pdb_data_from_pdb(self, pdb_code):
        url = 'http://www.rcsb.org/pdb/rest/getEntityInfo?structureId=' + \
            pdb_code
        return url

    def get_data_from_pdb(self, pdb_code, pdb_data=False, mol_data=False):
        # Step 1: ask the pdb for the organism, chain, macromolecule_name,
        # uniprot id
        if pdb_data:
    ...
    def rank_by_resolution(self, res_map):
        ...
        for pdb_code in res_map:
            if res_map[pdb_code]['resolution']:
                ranked_map.append((pdb_code,
                                   float(res_map[pdb_code]['resolution'])))
    ...
    def assign_variables(self, polymer):
        ...
        try:
            ...
            uniprot_id = None
        return (str(organism), str(chain), str(macromolecule_name),
                str(uniprot_id))
    ...
    def parse_mol_xml(self, data):
        ...
        if isinstance(polymers, list):
            for polymer in polymers:
                organism, chain, macromolecule_name, uniprot_id = \
                    self.assign_variables(polymer)
