...
class MutationListGenerator:
    def __init__(self, sequence_annotations, dna_sequences, aln_diff,
                 pdb_code='', psiblast=''):
    ...
    def _get_data(self):
        ...
        elif self.ddG_results_filepath and not self.llikelihood_filepath:
            self._capture_ddG_data()
        elif not self.ddG_results_filepath and self.llikelihood_filepath:
            self._capture_llikelihood_data()
        else:
            print ("Program should have quit with exit code 1 in the event no "
                   "data files were specified... something weird happended.")
        ...
        for i in range(len(self.ddG_data)):
            ...
            if len(cleaned[1].split("-")) < 2:    # ignore first line
                ...
                wt_res, position, mut_res = \
                    mutation[0], mutation[1:-1], mutation[-1]
    ...
    def write_csv_output(self):
        ...
        out_filename = \
            self.pdb_dir + '/' + self.pdb_code + '_mutation_list.csv'
        output = open(out_filename, 'w')
        for key in self.mutation_map:
            start_site = '100'
            wild_type_residue = key[0]
            position = str(int(key[1]) + int(self.aln_diff[0][1]))
        ...
        for key in self.ddG_data_map:
            ...
            output.write(gene_name + '    ' + dna_sequence + '    ' +
                         start_site + '    ' + wild_type_residue + '    ' +
                         position + '    ' + codons + '\n')
    ...
    def _build_mut_seq_list(self):
        ...
        for key in self.ddG_data_map:
            if (key[1], key[2]) not in mutation_map:
                mutation_map[(key[1], key[2])] = \
                    set([settings.E_COLI_CODON_USAGE[key[3]][0][0]])
            else:
                mutation_map[(key[1], key[2])].add(
                    settings.E_COLI_CODON_USAGE[key[3]][0][0])
        print mutation_map
...
class SequenceGetter:
    ...
    def _write_fasta(self, seq_list):
        for i in range(len(seq_list)):
            ...
            outfile = self.pdb_dir + '/' + seq_id + '.fasta'
    ...
    def get_DNA_sequence(self):
        for gene in self.genes:
            dna_sequence = BioModules.NCBI_gene_fetcher(self.organism, gene,
                                                        100)
    ...
    def get_protein_sequence(self):
        for i in range(len(self.dna_sequences)):
            for sequence in self.dna_sequences[i]:
                self.protein_sequences.append(
                    BioModules.dna2protein(sequence[1][100:-100]))
            seq_list = []
